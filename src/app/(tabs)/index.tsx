...
const ExploreTab = () => {
    ...
    const historicalLandmarks = useLandmarkStore(v => v.landmarks);
    ...
    const handleSearchResultPress = (id: number) => {
        const landmark = historicalLandmarks.find(v => v.id == id);
        if (landmark) {
            setSelectedLandmark(landmark);
            centerMapOnCoord([landmark.longitude, landmark.latitude], 16);
    ...
    return (
        <VStack style={styles.page}>
                    ...
                    >
                        <Box style={styles.markerContainer}>
                            ...
                            >
                                <Ionicons
                                    name='location-sharp'
                                    size={selectedLandmark?.id === landmark.id ? 36 : 28}
                                    color={selectedLandmark?.id === landmark.id ? '#ff4444' : '#4CAF50'}
                                />
                            </Pressable>
                            <Box style={[
                                styles.markerLabel,
                                selectedLandmark?.id === landmark.id && styles.markerLabelSelected
                            ]}>
            ...
            <Actionsheet isOpen={selectedLandmark != null} onClose={handleCloseBottomSheet} snapPoints={[45]} >
                <ActionsheetBackdrop />
                <ActionsheetContent >
                    ...
                    <ScrollView className='w-full'>
                        {
                            selectedLandmark != null && (
                                <Box className='w-full gap-4 '>
                                    ...
                                    <Button
                                        onPress={() => {
                                            console.log('Add to itinerary:', selectedLandmark?.name);
                                        }}
                                    >
